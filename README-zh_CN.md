...
- **计数**（`-c`）：`序列条数 字节数 时长微秒 平均字节/秒`

使用 `-q` 可只输出附加行，不打印序列。

柱状序列在记录时即流式输出到 stdout（无缓冲、逐值刷新），管道与实时监控
无需等待下载结束。

默认记录完整时间线，包括连接与服务器延迟（块模式 TTFB、间隔模式前导零）。
使用 **`-d` / `--delayed=TIMEOUT`** 时，数据到达前的等待不会进入序列；`-p`
的 offset 为剥离的延迟（秒）。`TIMEOUT` 为必填：`-d0` 无限等待首字节；正数
（如 `-d5s`）在超时后仍无数据也会进入数据阶段。
...
| `-w`, `--window=NUM[um][s]` | 从连接开始计时，超时后停止 |
...
| `-c`, `--count` | 输出摘要行：条数、字节、时长（微秒）、平均 B/s |
| `-d`, `--delayed=NUM[um][s]` | 省略数据前等待；`-d0` 无限等待首字节 |
...
- **未使用 `-d`**：offset 为 0；块模式仍将 TTFB 作为第一个柱。
- **使用 `-d`**：offset 为剥离的数据前延迟（**秒**），从连接开始到数据阶段
  （首字节到达或 `-d` 超时触发）。
...
```bash
getbar -i 100ms -d0 -p 2 https://example.com/file    # 无限等待首字节
getbar -i 100ms -d5s -w 30s https://example.com/file # 空闲 5 s 后进入数据阶段
getbar -b 64k -d0 -s 256k https://example.com/file   # 块模式不含 TTFB 柱
```

`-d` 必须带超时参数；无限等待请写 `-d0`（不能省略参数）。正数超时会在尚无
数据时强制进入数据阶段，间隔模式可能输出零桶，`-p` offset 反映配置的等待时间。
